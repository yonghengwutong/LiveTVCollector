...
BRANCH = "main"
BASE_PATH = os.path.abspath("BugsfreeStreams/StreamsTV-MIXED")
FINAL_M3U_FILE = os.path.abspath("BugsfreeStreams/Output/StreamLinks-MIXED.m3u")
MAX_SMIXEDEAMS = 500
MAX_SMIXEDEAMS_PER_SOURCE = 50
...
DEFAULT_SOURCE = "https://raw.githubusercontent.com/bugsfreeweb/LiveTVCollector/refs/heads/main/LiveTV/Mixed/LiveTV.m3u"

# Source M3U playlist(s) - primary and fallbacks
SOURCES = [DEFAULT_SOURCE]
FALLBACK_SOURCES = [
    "https://raw.githubusercontent.com/bugsfreeweb/LiveTVCollector/refs/heads/main/LiveTV/Mixed/LiveTV.m3u",    
]
...
def get_variant_streams(master_url):
    ...
    try:
        ...
        if "#EXT-X-SMIXEDEAM-INF" in content:
            lines = content.splitlines()
            for i, line in enumerate(lines):
                if line.startswith("#EXT-X-SMIXEDEAM-INF"):
...
def parse_m3u(content):
    ...
    return entries[:MAX_SMIXEDEAMS_PER_SOURCE]  # Limit per source
...
def main():
    ...
    for extinf, url in all_entries:
        if len(unique_streams) >= MAX_SMIXEDEAMS:
            logger.info(f"Reached MAX_SMIXEDEAMS limit: {MAX_SMIXEDEAMS}")
            break
    ...
    if not unique_streams:
        ...
        variants = get_variant_streams(FALLBACK_SMIXEDEAM["url"])
        unique_streams[FALLBACK_SMIXEDEAM["name"]] = (FALLBACK_SMIXEDEAM["extinf"], FALLBACK_SMIXEDEAM["url"], variants)
    ...
    for channel_name, (extinf, original_url, variants) in unique_streams.items():
        github_url = f"https://raw.githubusercontent.com/{REPO_OWNER}/{REPO_NAME}/refs/heads/{BRANCH}/BugsfreeStreams/StreamsTV-MIXED/{channel_name}.m3u8"
        ...
        for variant in variants:
            ...
            m3u8_content.append(f"#EXT-X-SMIXEDEAM-INF:PROGRAM-ID=1,BANDWIDTH={bandwidth},RESOLUTION={resolution}")
