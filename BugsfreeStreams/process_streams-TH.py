...
BRANCH = "main"
BASE_PATH = os.path.abspath("BugsfreeStreams/StreamsTV-TH")
FINAL_M3U_FILE = os.path.abspath("BugsfreeStreams/Output/StreamLinks-TH.m3u")
MAX_STHEAMS = 500
MAX_STHEAMS_PER_SOURCE = 50
...
DEFAULT_SOURCE = "https://iptv-org.github.io/iptv/countries/th.m3u"

# Source M3U playlist(s) - primary and fallbacks
SOURCES = [DEFAULT_SOURCE]
FALLBACK_SOURCES = [
    "https://raw.githubusercontent.com/bugsfreeweb/LiveTVCollector/refs/heads/main/LiveTV/Thailand/LiveTV.m3u",    
]
...
def get_variant_streams(master_url):
    ...
    try:
        ...
        if "#EXT-X-STHEAM-INF" in content:
            lines = content.splitlines()
            for i, line in enumerate(lines):
                if line.startswith("#EXT-X-STHEAM-INF"):
...
def parse_m3u(content):
    ...
    return entries[:MAX_STHEAMS_PER_SOURCE]  # Limit per source
...
def main():
    ...
    for extinf, url in all_entries:
        if len(unique_streams) >= MAX_STHEAMS:
            logger.info(f"Reached MAX_STHEAMS limit: {MAX_STHEAMS}")
            break
    ...
    if not unique_streams:
        ...
        variants = get_variant_streams(FALLBACK_STHEAM["url"])
        unique_streams[FALLBACK_STHEAM["name"]] = (FALLBACK_STHEAM["extinf"], FALLBACK_STHEAM["url"], variants)
    ...
    for channel_name, (extinf, original_url, variants) in unique_streams.items():
        github_url = f"https://raw.githubusercontent.com/{REPO_OWNER}/{REPO_NAME}/refs/heads/{BRANCH}/BugsfreeStreams/StreamsTV-TH/{channel_name}.m3u8"
        ...
        for variant in variants:
            ...
            m3u8_content.append(f"#EXT-X-STHEAM-INF:PROGRAM-ID=1,BANDWIDTH={bandwidth},RESOLUTION={resolution}")
