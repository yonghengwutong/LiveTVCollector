...
DEFAULT_SOURCE = "https://iptv-org.github.io/iptv/countries/br.m3u"

# Source M3U playlist(s) - primary and fallbacks
SOURCES = [DEFAULT_SOURCE]
FALLBACK_SOURCES = [
    "https://raw.githubusercontent.com/bugsfreeweb/LiveTVCollector/refs/heads/main/LiveTV/Brazil/LiveTV.m3u",    
]
...
def get_variant_streams(master_url):
    ...
    try:
        ...
        if "#EXT-X-SBREAM-INF" in content:
            lines = content.splitlines()
            for i, line in enumerate(lines):
                if line.startswith("#EXT-X-SBREAM-INF"):
...
def parse_m3u(content):
    ...
    return entries[:MAX_SBREAMS_PER_SOURCE]  # Limit per source
...
def main():
    ...
    for extinf, url in all_entries:
        if len(unique_streams) >= MAX_SBREAMS:
            logger.info(f"Reached MAX_SBREAMS limit: {MAX_SBREAMS}")
            break
    ...
    if not unique_streams:
        ...
        variants = get_variant_streams(FALLBACK_SBREAM["url"])
        unique_streams[FALLBACK_SBREAM["name"]] = (FALLBACK_SBREAM["extinf"], FALLBACK_SBREAM["url"], variants)
    ...
    for channel_name, (extinf, original_url, variants) in unique_streams.items():
        github_url = f"https://raw.githubusercontent.com/{REPO_OWNER}/{REPO_NAME}/refs/heads/{BRANCH}/BugsfreeStreams/StreamsTV-BR/{channel_name}.m3u8"
        ...
        for variant in variants:
            ...
            m3u8_content.append(f"#EXT-X-SBREAM-INF:PROGRAM-ID=1,BANDWIDTH={bandwidth},RESOLUTION={resolution}")
